...
- [ ] Requirement 1: プリセットテーマの即時適用・保存復元・滑らかな切替を提供する。
- [ ] Requirement 2: アクセント/中間トーンを安全なコントラストで微調整できるようにする。
- [ ] Requirement 3: OS設定に連動したライト/ダーク自動切替と印刷時の配慮を実装する。
- [ ] Requirement 4: 色覚多様性モードと凡例表示で誰でも識別できる表示を整える。

### Epic 2: カンバン基本操作（カード/列/ドラッグ&ドロップ/編集）
- [ ] Requirement 5: 列間ドラッグ&ドロップと無効ドロップ防止、取り消しを実装する。
- [ ] Requirement 6: インライン作成/編集と自動保存、連続作成モードを提供する。
- [ ] Requirement 7: キーボードショートカットによる移動・編集・フォーカス表示を整備する。
- [ ] Requirement 8: 操作履歴を保持し、取り消し/やり直しを安心して行えるようにする。

### Epic 3: 視覚メタファと情報表現（タグ/期限/担当/チェックリスト）
- [ ] Requirement 9: タグの色/テキスト表示と溢れ制御、一貫性を確保する。
- [ ] Requirement 10: 期限の近さ/超過を穏やかに可視化し、色覚配慮モードに適応させる。
- [ ] Requirement 11: 担当者アバター/未割当バッジとプライバシー保護を実装する。
- [ ] Requirement 12: チェックリスト進捗バッジと完了表示、並び順同期を整える。

### Epic 4: 検索・フィルタ・並び替え・ビュー保存
- [ ] Requirement 13: タイトル/説明/タグの高速全文検索と正規化マッチングを実現する。
- [ ] Requirement 14: 担当/タグ/期限フィルタと条件チップ、空状態表示を整える。
- [ ] Requirement 15: 期限/優先度/更新日の並び替えと手動順の優先制御を提供する。
- [ ] Requirement 16: 条件の保存/共有ビューと権限制御、上書き管理を実装する。

### Epic 5: レイアウトとレスポンシブ/モバイル操作/スイムレーン
- [ ] Requirement 17: 自動列幅最適化と手動リサイズ、設定保持を行う。
- [ ] Requirement 18: スクロールスナップ・オートスクロール・位置復元を実装する。
- [ ] Requirement 19: モバイルジェスチャ対応とアクセシビリティ調整を提供する。
- [ ] Requirement 20: スイムレーンによるグルーピングと折りたたみ、保存連携を整える。

### Epic 6: WIP（仕掛かり）制限とポリシー
- [ ] Requirement 21: 列単位のWIP上限設定と監査ログ記録を実装する。
- [ ] Requirement 22: 穏やかな警告と制限下の操作制御、色覚配慮対応を提供する。
- [ ] Requirement 23: 強制/例外許可ポリシー切替と理由記録、テンプレ既定設定を実装する。
- [ ] Requirement 24: WIPヒートマップと凡例、配慮モード切替を整える。

### Epic 7: フィードバック/アニメーション/トースト
- [ ] Requirement 25: 控えめなドラッグアニメーションと低負荷モード対応を実装する。
- [ ] Requirement 26: 保存/失敗/制限を伝える非破壊トーストと読み上げ対応を提供する。
- [ ] Requirement 27: 危険操作の確認ダイアログと復元手段、アクセシビリティ配慮を整える。
- [ ] Requirement 28: 操作音のオン/オフ切替と環境依存のフィードバック制御を実装する。

### Epic 8: パフォーマンス/永続化/オフライン・状態保持
- [ ] Requirement 29: 大量カードでも滑らかな操作を維持する仮想化と軽量表示を実現する。
- [ ] Requirement 30: 楽観的更新と差分解決、再同期フィードバックを提供する。
- [ ] Requirement 31: オフライン作業キューと再接続同期、帯域配慮を実装する。
- [ ] Requirement 32: ビュー/位置の復元と端末間同期、プライバシーモード対応を整える。
